...
def _evaluate(args):
    if not args.output:
        output_dirpath = os.path.dirname(args.dataset)
    else:
        output_dirpath = args.output
    if not os.path.exists(output_dirpath):
        logger.info('Creating directory {}'.format(output_dirpath))
        ...
        logger.info('Saving to directory {}'.format(output_dirpath))
    evaluator.evaluate(output_dirpath, args.dataset, args.prototypes, args.space)
...
def main():
    ...
    parser_evaluate.add_argument(
        '-d', '--dataset', required=True, help='absolute filepath to dataset. '
        'Supported datasets: mcrae.txt, pado.txt, ferretti-instruments.txt, '
        'ferretti-locations.txt')
    parser_evaluate.add_argument(
        '-p', '--prototypes', required=True, help='absolute filepath to '
        'prototypes.')
    parser_evaluate.add_argument(
        '-s', '--space', required=True, help='absolute filepath to '
        'distributional space. This must be in npy or npz format.')
